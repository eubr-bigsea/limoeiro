...
# Class to implement the collection data engine.
    ...
    # Method to do a diff check between two list objects.
        ...
        for new in new_dict[k]:
                ...
                if new['name'] == current['name']:
                    ...
                    for n in new.keys():
                        ...
                        if (new[n] != current[n]):
                            ...
                            self.log.log_diff_list_element (obj_type, current_dict["id"], current_dict["fully_qualified_name"], k, n, current[n], new[n])
            ...
            if match == False:
                ...
                # Log the difference found.
                self.log.log_element_added (obj_type, current_dict["id"], current_dict["fully_qualified_name"], k, new['name'])

        # Iterate over the old values and search for their match in the new values.
        for current in current_dict[k]:
            match = False
            for new in new_dict[k]:
                if new['name'] == current['name']:
            ...
            if match == False:
                ...
                # Log the difference found.
                self.log.log_element_removed (obj_type, current_dict["id"], current_dict["fully_qualified_name"], k, current['name'])
    ...
    # Method to do a diff check between two elements.
        ...
        if (new_value != current_value):
            diff = True
            # Log the difference found.
            self.log.log_diff_field(obj_type, current_dict["id"], current_dict["fully_qualified_name"], k, current_value, new_value)

        return diff

    def diff_objects (self, new_dict, current_dict, obj_type):
    # Method to do a diff check between two objects, and return the object with the new values replaced.
        ...
        if (diff):
            ...
            new_version = ".".join([str(int(version_arr[0])+1), version_arr[1], version_arr[2]])
