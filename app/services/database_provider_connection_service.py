...
class DatabaseProviderConnectionService(BaseService):
    ...
    async def add(
        self, database_provider_connection_data: DatabaseProviderConnectionCreateSchema
        ...
        """
        database_provider_connection = DatabaseProviderConnection(**database_provider_connection_data.model_dump(
            exclude_unset=True))
    ...
    async def delete(self, database_provider_connection_id: UUID) -> DatabaseProviderConnection:
        """
        ...
        """
        database_provider_connection = await self.get(database_provider_connection_id)
    ...
    async def update(self,
        ...
        database_provider_connection_data: DatabaseProviderConnection) -> DatabaseProviderConnection:
        """
    ...
    async def find(self,
        query_options: DatabaseProviderConnectionQuerySchema
        ...
        filters = self.get_filters(DatabaseProviderConnection, filter_opts, query_options)
        ...
        if (
            query_options.sort_by and
                hasattr(DatabaseProviderConnection, query_options.sort_by)
        ):
            order_func = asc if query_options.sort_order != "desc" else desc
            query = query.order_by(
                order_func(getattr(DatabaseProviderConnection, query_options.sort_by)))
        rows = (
            await self.session.execute(query.offset(offset).limit(limit))
        ).scalars().unique().all()

        count_query = select(func.count()).select_from(
            query.selectable.with_only_columns(DatabaseProviderConnection.id)
        )
    ...
    async def get(self, database_provider_connection_id: UUID) -> DatabaseProviderConnection:
        """
        ...
        result = await self.session.execute(
            ...
            .filter(DatabaseProviderConnection.id == database_provider_connection_id))
