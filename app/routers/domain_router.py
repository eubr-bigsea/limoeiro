...
async def add_domain(
    domain_data: DomainCreateSchema, db: AsyncSession = Depends(get_session)
) -> DomainItemSchema:
    """
    Adiciona uma instância da classe Domain.
    """
    return await DomainService(db).add(domain_data)
...
async def delete_domains(
    domain_id: UUID, db: AsyncSession = Depends(get_session)
):
    """
...
async def update_domains(
    ...
    """
    return await DomainService(db).update(domain_id, domain_data)
...
async def get_domain(
    ...
    domain = await DomainService(db).get(domain_id)
    if domain is None:
